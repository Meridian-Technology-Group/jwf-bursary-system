...
| **D13-1a** | **Amends D13-1 (Brian, 2026-08-14): the account reference and the application reference are separate identifiers, and `BursaryAccount.feesAccountCode` is removed as redundant.** `BursaryAccount.reference` becomes the free-text external fees-system code (`TS-SMITH05-Smith, Bob` — no academic year, stable across years); `Application.reference` stays the per-year label with the D13-1 default format. Both non-unique. **The "ROLLING_OVER carries the reference forward" rule is dropped** — it only existed because D13-1 conflated the two. Rationale and the three-PR split in [`sprint-01-implementation-plan.md` §5 C4](sprint-01-implementation-plan.md). |
