...
status: complete — all 23 WPs merged to staging (PRs #295–#316, 2026-08-16)
opened: 2026-08-16
related:
...
- Staging browser pass of the full assessment path before Charlotte's
  session.
- **Epic complete (all 23 WPs merged, #295–#316).** Throwaway fixtures left
  on nonprod for your staging pass: parent `brian+K7m9Qx2P@meridiantech.group`
  (pw `E14-a1-throwaway!`) with the two-child setup (Melody SUBMITTED/AWARDED
  + SecondKid draft), round `2087/88` (close date pushed to 2087 + an
  NA_NEXT_WINTER window row), test assessor `brian.assessor@jwf-bursary.test`
  (`E14-assessor-check!`). Say the word and it all gets purged (audit rows
  stay — append-only).
...
- **B3 selectors + Settings templates** — verified on the staging deploy
  post-merge (situation selectors live; 5 variants listed + locked).
- **D2 live send (staging deploy, 2026-08-16):** NA_NEXT_WINTER window
  (submit-by 03/03/2087) stored on the throwaway 2087/88 round whose NEW
  default is NULL → quick invite (NEW, Whitgift) → the received email says
  *"the deadline for submitting your completed application is 03/03/2087"*
  (the window value; the closeDate fallback would have been 19/08/2087) with
  the link expiry separate (15/09/2026) and the B3 NEW_WS body carried.
  Probe invitation + auth user deleted after.
