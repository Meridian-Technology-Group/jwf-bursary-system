...
> **Outcomes (2026-08-14, #288):**
>
> - **One read, two consumers.** A1's `readObjectHead` (a signed-URL Range
>   request) was widened from 64 bytes to 64 KB and its buffer now feeds both
>   `sniffContentType` and the digest. A test asserts it is called exactly once.
> - **The digest is a prefix hash, deliberately:**
>   `sha256("<exact stored byte length>:" || first 64 KB)`. Hashing whole files
>   would pull up to 20 MB back through the route — precisely what the presigned
>   transport exists to avoid. **For any file under 64 KB (most UC statements)
>   the "prefix" is the whole file, so the digest is exact**; only larger files
>   use a true prefix, and those must match on byte length *as well as* the first
>   64 KB to collide. It is duplicate *detection*, not proof of identity.
> - **Reject for UC, warn elsewhere.** The reject fires only when the incoming
>   slot is a UC slot **and** the existing match sits in a UC slot. The same file
>   in `HOUSING_BENEFIT` then `UC_MONTHLY_2` warns rather than blocks — refusing
>   it would punish an honest applicant whose one letter evidences two lines.
> - **⚠️ `ucMonthlyDocumentIds` changed shape** from `string[]` to positional
>   `(string | null)[]`, normalised to exactly three entries on mount. Existing
>   nonprod blobs with a single id keep it in position 1 — **spot-check an
>   in-progress application after this lands.**
> - Repeat-slot UI and the 409 path are **unit-tested only** — they want a
...
Each WP fixed its own instance. But "fix the cause" is **not** one change, and
unmounting is **not** obviously the goal — the CSS-hiding is partly deliberate,
because it preserves what an applicant typed across an accidental toggle.
Decomposed into **F11a** (unique ids — small, safe, do it), **F11b** (a product
decision that may require no code at all), and **F11c** (a per-form pattern).
See the table below.

> **Correction (2026-08-14):** this was first filed as a single L-sized "fix the
> cause" item implying that hidden branches should unmount. Investigation showed
> that would fix the DOM defects but **not** the stale-blob one — `shouldUnregister`
> is set nowhere, so react-hook-form's default retains unmounted values — while
> also discarding input the current design deliberately preserves.
...
| ~~**F11a**~~ ✅ | F2 | **DONE — shipped in [#289](https://github.com/Meridian-Technology-Group/jwf-bursary-system/pull/289), commit 2.** A `useUploadControlIds()` hook using `React.useId()`, called unconditionally in both variants (deliberately *before* `SingleFileUpload`'s `variant === "inline"` early return, so hook order stays stable). 31 insertions / 4 deletions. `useId` is hydration-stable by construction, so no guard was needed. Re-grep confirmed nothing outside the module referenced the old id strings. **Give upload controls unique DOM ids.** `file-upload.tsx` derives `inputId`/`descId` from the slot (lines 327-328 and 650-651); use `React.useId()` instead — the pattern is already established at `src/components/ui/form.tsx:79`. **Verified contained:** nothing outside `file-upload.tsx` references those id strings. Kills the "visible label opens a hidden input" class permanently, for any future form too. Note the component's existing `aria-hidden` + `pointer-events-none` do **not** prevent this — label-for activation is neither a pointer event on the input nor blocked by `aria-hidden`, which is how it survived review. | **S** |
| **F11b** | D3 | **Decide the product question before writing code:** should typing into a branch, toggling it off, and toggling back **preserve** what was typed? Today's answer is **yes**, and that is arguably why the CSS-hiding exists. If yes stays, **D3's rule guards are the correct permanent design, not a stopgap**, and nothing further is needed. If no, it means unmounting **plus** `shouldUnregister: true` across 10 `useForm` call sites — `shouldUnregister` is currently set nowhere, so RHF's default keeps unmounted values and unmounting alone would **not** fix stale blobs. Trading this without deciding risks swapping one data-loss complaint for another, from a client who has already lost work three times. | **L** (only if "no") |
| **F11c** | F2 | **Render a control whenever it holds data, even off-branch** — so a document can never be stranded behind a branch the applicant later switches away from. F2 already did this for the ILR control; generalising is a per-form pattern, not a component change. | M |
