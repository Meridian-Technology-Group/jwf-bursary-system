...
- [ ] **Editing under a live assessment discards it (state-model §4/§6.5/§7.2, D-G6/D3).** With an application whose **assessment has begun** (*In Progress*) or is *Paused*, make a real change to a section on the applicant's behalf: the **assessment resets to Not Started** (any draft outcome and the missing-documents/pause clock are cleared), the form **stays Submitted with its original submission date**, and the History shows an **"Assessment discarded (form changed)"** entry naming the changed fields. Confirm the assessor must **re-run** the assessment, and that a save which changes nothing does **not** reset it.
