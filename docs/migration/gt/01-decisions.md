...
| C6 | **Child benefit inside "income support"** on new-application assessments: confirm per flagged account. | Import as entered under income support (`benefits.other`), flag | Flagged accounts | M2 pack. **24 Sep: moves no figure** — child benefit and income support both sum into income, so the ruling only relabels. Evidence for her: on the new-award form GT's own "ADD BACK IN CHILD SUPPORT TOTAL" equals income support on 25 of 36. |
| C7 | **Other properties ≠ 0:** split between second property and "multiple portfolio" per flagged account. | Import the aggregate as `second` with portfolio type `DOUBLE`; a portfolio goes in `other` with `MULTIPLE`. **Corrected 24 Sep:** the engine reads `DOUBLE` from `second` and `MULTIPLE` from `other` (`profiling.ts` `relevantProperty`), so the original default (`other` + `DOUBLE`) would have categorised all 24 accounts off an empty property. Equity sums all three slots either way. | Flagged accounts | M2 pack |
...
| C12 | **Loan and lease debt:** GT records *yearly repayments*; the current model expects *outstanding balances* (credit cards are balances in both). How should a GT yearly figure become a balance? | Load credit-card and Foundation debt as-is; load loans/leases as yearly repayment × schooling years remaining (so the model's per-year spread returns GT's yearly figure); list every such account in the pack (about half of all accounts carry some debt repayment) | Nothing: one global rule; a change re-runs the build | M2 pack. **Settled 24 Sep (Brian): take loan/lease figures as GT entered them** — premise of Q2 was wrong, so her Q2 answer does not override this; the covering email corrects the question. (1) GT itself never treated the figure as yearly: its "Debts other than foundation" = CC balance + loan figure + lease figure, and its "Derived Yearly Debt Repayments" = that ÷ schooling years, on 144/144 accounts. (2) The engine now spreads debt over a fixed 5 years (8 Sep respec, `DEBT_REPAYMENT_YEARS`), so × schooling years no longer returns GT's yearly figure; × 5 would. Built as a switch (`--debt as-entered\|x5`). Impact is small: the recommendation moves on 5 accounts. |
...
| C16 | **Mortgage-free rent add-back** (new, 24 Sep). The model offers "Full — mortgage-free (+100%)" rent add-back, which an assessor would select for a family owning its home outright. The mapping doc did not address it. | Apply `FULL_MORTGAGE_FREE` where the home has a value and no mortgage (4 accounts) | Nothing: global rule | **Settled 24 Sep (Brian): apply it**; she is asked to confirm in the report's covering email. |
