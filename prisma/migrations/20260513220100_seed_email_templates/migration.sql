-- Idempotently seed the 8 transactional email templates the application
-- depends on. The standalone prisma seed script (`prisma db seed`) is
-- destructive (it deleteMany's every table before recreating), so it
-- cannot safely be run against staging or production. This migration
-- inserts each template only when no row with that type already exists,
-- so it is safe to re-run and safe to merge into prod even when the
-- INVITE_STAFF row is already populated.
--
-- Template bodies are kept in sync with prisma/seed-data/email-templates.ts.
-- If you update a body, update both locations and write a follow-up
-- migration if the change must reach environments that have already
...
SELECT gen_random_uuid(),
       'INVITATION',
       'You are invited to apply for a bursary — {{child_name}}',
...
John Whitgift Foundation$body$,
       '["applicant_name","child_name","school","round_year","registration_link","deadline"]'::jsonb,
       now()
WHERE NOT EXISTS (SELECT 1 FROM public.email_templates WHERE type = 'INVITATION');

-- ── CONFIRMATION ───────────────────────────────────────────────────────────
INSERT INTO public.email_templates (id, type, subject, body, merge_fields, updated_at)
SELECT gen_random_uuid(),
       'CONFIRMATION',
       'Bursary application received — {{child_name}}',
$body$Dear {{applicant_name}},
...
The Bursary Office
John Whitgift Foundation$body$,
       '["applicant_name","child_name","school","reference","submission_date"]'::jsonb,
       now()
WHERE NOT EXISTS (SELECT 1 FROM public.email_templates WHERE type = 'CONFIRMATION');
...
The Bursary Office
John Whitgift Foundation$body$,
       '["applicant_name","child_name","reference","missing_documents","deadline"]'::jsonb,
       now()
...
John Whitgift Foundation$body$,
       ...
       now()
WHERE NOT EXISTS (SELECT 1 FROM public.email_templates WHERE type = 'OUTCOME_QUALIFIES');
...
John Whitgift Foundation$body$,
       '["applicant_name","child_name","school","reference","academic_year"]'::jsonb,
       now()
WHERE NOT EXISTS (SELECT 1 FROM public.email_templates WHERE type = 'OUTCOME_DNQ');

-- ── REASSESSMENT ───────────────────────────────────────────────────────────
INSERT INTO public.email_templates (id, type, subject, body, merge_fields, updated_at)
SELECT gen_random_uuid(),
       'REASSESSMENT',
       'Annual bursary re-assessment — {{child_name}}',
$body$Dear {{applicant_name}},
...
John Whitgift Foundation$body$,
       '["applicant_name","child_name","school","round_year","registration_link","deadline"]'::jsonb,
       now()
WHERE NOT EXISTS (SELECT 1 FROM public.email_templates WHERE type = 'REASSESSMENT');
...
The Bursary Office
John Whitgift Foundation$body$,
       '["applicant_name","child_name","reference","deadline"]'::jsonb,
       now()
WHERE NOT EXISTS (SELECT 1 FROM public.email_templates WHERE type = 'REMINDER');

-- ── INVITE_STAFF ───────────────────────────────────────────────────────────
INSERT INTO public.email_templates (id, type, subject, body, merge_fields, updated_at)
SELECT gen_random_uuid(),
       'INVITE_STAFF',
       'You''ve been invited to the JWF Bursary Assessment System',
...
The Bursary Office
John Whitgift Foundation$body$,
       '["first_name","role","registration_link"]'::jsonb,
       now()
WHERE NOT EXISTS (SELECT 1 FROM public.email_templates WHERE type = 'INVITE_STAFF');
