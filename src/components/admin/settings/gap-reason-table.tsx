...
/**
 * CALC-11 — Interactive gap-reason management table.
 * Mirrors `reason-code-table.tsx` exactly (same inline-edit / add / deprecate
 * mechanics) — GapReason has the identical shape (code, label, isDeprecated,
 * sortOrder), minus the reason-code "category" grouping which doesn't apply
 * to gap reasons (Appendix E).
 */
...
import { upsertGapReasonAction } from "@/app/(admin)/settings/actions";
import type { GapReasonRow } from "@/lib/db/queries/reference-tables";
...
function EditableRow({ row }: EditableRowProps) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await upsertGapReasonAction(fd);
      if (result.success) {
        setEditing(false);
      } else {
  ...
  if (!editing) {
    return (
      <TableRow className={row.isDeprecated ? "opacity-60" : undefined}>
        ...
        <TableCell className="text-sm text-slate-700 max-w-md">{row.label}</TableCell>
        <TableCell>
          {row.isDeprecated ? (
...
function AddGapReasonRow({ onDone }: { onDone: () => void }) {
    ...
    <TableRow className="bg-blue-50/40">
      ...
      <TableCell>
        <Input
          ...
          onChange={(e) => setLabel(e.target.value)}
          className="h-8 text-sm"
          placeholder="Description of gap reason"
          aria-label="Label"
        />
      </TableCell>
      <TableCell>
...
interface GapReasonTableProps {
  gapReasons: GapReasonRow[];
}

export function GapReasonTable({ gapReasons }: GapReasonTableProps) {
  const [showAdd, setShowAdd] = React.useState(false);
  const [filterActive, setFilterActive] = React.useState<
    "all" | "active" | "deprecated"
  >("all");

  const filtered = gapReasons.filter((gr) => {
    if (filterActive === "active") return !gr.isDeprecated;
    if (filterActive === "deprecated") return gr.isDeprecated;
    return true;
  });
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <Table>
          ...
          <TableBody>
            {showAdd && <AddGapReasonRow onDone={() => setShowAdd(false)} />}
            {filtered.map((gr) => (
              <EditableRow key={gr.id} row={gr} />
            ))}
            {filtered.length === 0 && !showAdd && (
              <TableRow>
                ...
                >
                  No gap reasons match the current filter.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <p className="text-xs text-slate-400">
        {gapReasons.filter((gr) => !gr.isDeprecated).length} active,{" "}
        {gapReasons.filter((gr) => gr.isDeprecated).length} deprecated
      </p>
    </div>
  );
}
